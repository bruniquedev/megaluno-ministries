...
$fileNameToStore = (new AppHelper())->StoreFileHelper($upload_dir,$thumbnail_dir,$isToresize,$max_width,$fileinput);
    ...
    $data->save();
     return redirect('manage-gallery')->with('success','Data saved sucessfully '); //create a session variable Success to store
     //amessage
}
 ...
 /**
    ...
    {
        $data = gallery::find($id); //so we will have to find a particular post  
        //here we are going to delete from the database

           //if gallery_images in db not equal to noimage.jpg
           if($data->filename != 'user.png'){
            Storage::delete('public/gallery_images/'.$data->filename);
            Storage::delete('public/gallery_images/thumbnails/'.$data->filename);
            }
            
        $data->delete();
        return redirect('manage-gallery')->with('success','Data deleted sucessfully '); //create a session variable Success to store
        //amessage
        //
    }
...
$fileNameToStore = (new AppHelper())->StoreFileHelper($upload_dir,$thumbnail_dir,$isToresize,$max_width,$fileinput);
           ...
           $data = gallery::find($id); //so we will have to find a particular data
            $data->text = $request->descriptiontext; //captured from form
...
(new AppHelper())->DeleteFileHelper($upload_dir,$thumbnail_dir,$data->filename); 
    ...
    //DB::update('update student set first_name = ?,last_name=?,city_name=?,email=? where id = ?',[$first_name,$last_name,$city_name,$email,$id]);
            return redirect('manage-gallery')->with('success','Data updated sucessfully '); //create a session variable Success to store
            //amessage
    }
