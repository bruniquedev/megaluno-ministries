...
public function index()
{
  ...
  $data =DB::select('select * from sociallinks order by id desc');
  		
  //for editting in the same form purposes
  $Data = array('socialtype' =>'','descriptiontext' =>'', 'id' =>0);
  //var_dump($data);
...
public function store(Request $request)
{
    ...
    $data->socialtype = $request->socialtype; //captured from form
    $data->descriptiontext = $request->descriptiontext; //captured from form
    $data->save();
     return redirect('manage-socialmedia')->with('success','Data saved sucessfully '); //create a session variable Success to store
     //amessage
}
 ...
 /**
    ...
    {
        $data = sociallinks::find($id); //so we will have to find a particular post  
        //here we are going to delete from the database
        $data->delete();
        return redirect('manage-socialmedia')->with('success','Data deleted sucessfully '); //create a session variable Success to store
        //amessage
        //
    }
 ...
 /**
    ...
    public function edit($id)
    {
        ...
        //pass data to page for editting
         //passing multiple data
          return view('pagesadmin.socialmedia')->with('DataToEdit',$Data)->with('DataInfo',$data);
   ...
   /**
    ...
    public function update(Request $request, $id)
    {
           ...
           $data = sociallinks::find($id); //so we will have to find a particular data
           $data->socialtype = $request->socialtype; //captured from form
           $data->descriptiontext = $request->descriptiontext; //captured from form
           $data->save();

    //DB::update('update student set first_name = ?,last_name=?,city_name=?,email=? where id = ?',[$first_name,$last_name,$city_name,$email,$id]);
            return redirect('manage-socialmedia')->with('success','Data updated sucessfully '); //create a session variable Success to store
            //amessage
    }
