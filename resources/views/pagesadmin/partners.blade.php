...
@if(!isset($DataToEdit['id']) || $DataToEdit['id']==0)
    <form role="form" method="post" action="{{ route('manage-partnerlogos.store') }}" name="FORM" enctype="multipart/form-data">
    @else
    <form action="{{ route('manage-partnerlogos.update', $DataToEdit['id']) }}" method="post" name="FORM" 
  ...
  <label for="name" id="name">Partner name</label> 
...
<input type="text" required="required" class="input-control" name="descriptiontext" id="descriptiontext" value="{{$DataToEdit['text']}}" />
 ...
 <div class="form-groupy">
<label for="name" id="MessageLabel">Add partner link</label>
  <div class="form-input-group">
<textarea required="required"  
 class="input-control" name="link" id="link"  rows="4">
 {{$DataToEdit['link']}}</textarea>
</div>
</div>
...
<td>{{$Info->text}}</td>
<td>
  <div id="custom-img" style="background-image: url('{{ asset("storage/partners_images/".$Info->filename) }}'); height: 80px;">
